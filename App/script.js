...
function generateBars(n = -1) {
	...
	for (let i = 0; i < n; i++) {
		bars.push('<div class="bar" id="' + i + '" style="height:' + Math.floor(15 + Math.random() * 98) + '%"></div>');
	}
...
async function SelectionSort() {
	...
	let container = document.getElementById("container");
			...
			// ???
			document.getElementsByClassName("comp")[0].innerHTML = "";
			// ???
	...
	Finished_Sorting();
	document.getElementsByClassName("comp")[0].innerHTML = "O(n<sup>2</sup>)";
}
...
async function BubbleSort() {
	...
	// document.getElementById("comp").innerHTML("Bubble Srt");
...
async function MergeSort() {
	...
	Finished_Sorting();
	document.getElementsByClassName("comp")[0].innerHTML = "O(nlog(n))";
}
